<#
        ...
        ATTENTION: If you would like use the repair function like chkdsk make sure that the parameter IgnoreLessGB isn't defined!
	    ...
	    This shrinks all disks in the specified share recursively while not processing disks which have less than 30% whitespace instead of the default 15%.
         ...
         .NOTES
         Script-Name  : Invoke-FslShrinkDisk.ps1
         Version      : 1.1
         Created Date : 25.03.2021
         Created by   : Tobias Zurstegen 
        ...
        ---------------------------------------------------------------------------------------------------------------------------------
                      Jim Moyle                Original
         25.03.2021   Tobias Zurstegen         Added Repair function for corrupted disks
...
function Optimize-OneDisk {
    ...
    PROCESS {
        ...
        if ( $IgnoreLessThanGB -and $originalSize -lt $IgnoreLessThanBytes -and $ ) {
        ...
        #Repair Volume
        try{
            $volume = Get-Volume -Partition $partInfo -ErrorAction Stop 
        }
        catch{    
            $volume = Get-Volume -ErrorAction Stop | Where-Object {
            $_.UniqueId -like "*$($partInfo.Guid)*"
            -or $_.Path -Like "*$($partInfo.Guid)*"
            -or $_.ObjectId -Like "*$($partInfo.Guid)*" }   
        }

        if($volume.HealthStatus -ne "Healthy") {
            Write-VhdOutput -DiskState "Parition was corrupt" -EndTime (Get-Date)
                
            $chkdsk = $false
            $timespan = (Get-Date).AddSeconds(240)
               
            while($chkdsk -eq $false -and $timespan -gt (Get-Date)) {
                try {
                    Get-Volume -Partition $partInfo -ErrorAction Stop | Repair-Volume -OfflineScanAndFix -ErrorAction Stop
                    $chkdsk = $true
                }
                catch {
                    try {
                        ...
                        $_.UniqueId -like "*$($partInfo.Guid)*"
                        -or $_.Path -Like "*$($partInfo.Guid)*"
                        -or $_.ObjectId -Like "*$($partInfo.Guid)*" } | Repair-Volume -OfflineScanAndFix -ErrorAction Stop
                        $chkdsk = $true
                    }
                    catch {
                        $chkdsk = $false
